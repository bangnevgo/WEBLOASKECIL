...
const HTML_FILE = path.resolve('public/tiktok-assets/tiktok-ipad-tilted.html');
...
async function recordIpadTikTok() {
  console.log('🚀 Merekam Video TikTok iPad POV dengan Presisi Kemiringan Layar 3D (Matrix3D Quad Warp)...');
  ...
  console.log(`🌐 Membuka player iPad 3D Tilted: file://${HTML_FILE}`);
  await page.goto(`file://${HTML_FILE}`, { waitUntil: 'load' });
  await page.waitForTimeout(1000);

  // Putar video di dalam layar iPad
  await page.evaluate(() => {
  ...
  console.log('🎥 Merekam alur 32.5 detik POV iPad miring presisi dengan pergerakan zoom...');
  ...
  if (video) {
    ...
    const rawMp4 = path.join(OUTPUT_DIR, 'ipad_tilted_raw.mp4');
    ...
    console.log(`\n🎉 SUKSES BESAR! File video iPad Pro Kemiringan Presisi telah tersimpan di:\n👉 ${DOWNLOADS_TARGET}`);
  }
}

recordIpadTikTok().catch(err => {
