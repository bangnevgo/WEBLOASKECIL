...
const HTML_FILE = path.resolve('public/tiktok-assets/tiktok-lifestyle-player.html');
const OUTPUT_DIR = path.resolve('public/tiktok-assets');
const RECORDINGS_TEMP = path.resolve('recordings-temp');
const DOWNLOADS_TARGET = '/Users/ding/Downloads/LOAS_TikTok_Lifestyle_POV_Final.mp4';
const AUDIO_TRACK = path.join(OUTPUT_DIR, 'user_audio.mp3');

async function recordLifestyleTikTok() {
  console.log('🚀 Merekam Video TikTok Lifestyle POV (Hands Holding Smartphone + Live Screen Zoom)...');
  ...
  console.log(`🌐 Membuka file lokal: file://${HTML_FILE}`);
  await page.goto(`file://${HTML_FILE}`, { waitUntil: 'load' });
  await page.waitForTimeout(1000);

  // Pastikan video di dalam layar HP mulai berputar
  await page.evaluate(() => {
    const vid = document.getElementById('liveScreenVid');
    if (vid) {
      vid.muted = true;
      vid.currentTime = 0;
  ...
  console.log('🎥 Merekam alur 31.5 detik POV pergerakan zoom...');
  ...
  if (video) {
    ...
    const rawMp4 = path.join(OUTPUT_DIR, 'lifestyle_pov_raw.mp4');
    ...
    console.log(`\n🎉 SUKSES BESAR! File video kreasi Lifestyle POV telah tersimpan di:\n👉 ${DOWNLOADS_TARGET}`);
  }
}

recordLifestyleTikTok().catch(err => {
